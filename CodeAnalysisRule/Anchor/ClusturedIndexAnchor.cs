...
namespace Tibre.CodeAnalysisRule.Anchor
{
    [ExportCodeAnalysisRuleAttribute(RuleId
            , "Anchor table must have a clustered index corresponding to the identity column."
            , Category = "Tibre.Rules.Design"
            , RuleScope = SqlRuleScope.Element)]
    public class ClusturedIndexAnchor : BaseAnchor
    {
        public const string RuleId = "Tibre.CodeAnalysisRule.Anchor.ClusturedIndexAnchor";
        ...
        protected override IEnumerable<SqlRuleProblem> OnAnalyze(string name, TSqlObject table)
        {
            ...
            if (clusteredIndex == null)
                yield return new SqlRuleProblem(string.Format("No clustered index for the anchor table {0}", name), table);
            else
            {
                ...
                if (uniqueClusturedIndex == null)
                    yield return new SqlRuleProblem(string.Format("Clustured index for the anchor table {0} is not unique ", name), table);
                else
                {
                    ...
                    if (columns.Count() > 1)
                        yield return new SqlRuleProblem(string.Format("The anchor table {0} has a clustered index but this index has more than one column.", name), table);

                    if (columns.Count(c => c.GetProperty<bool>(Column.IsIdentity)) == 0)
                        yield return new SqlRuleProblem(string.Format("The anchor table {0} has a clustered index but this index doesn't include the identity column.", name), table);
                }
            }
        }
    }
